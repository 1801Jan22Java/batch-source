...
public class EditRequestServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//get the id of the manager who will be resolving the request
		int id = Integer.parseInt(request.getParameter("id"));
		//save the manager's change and notes
		String status = request.getParameter("status");
		String notes = request.getParameter("notes");
		RequestDao rd = new RequestDaoImpl();
		//if any of the fields are changed, then the manager's id will be added as the resolver
		boolean edited = false;
		//based on the what the manager wants to do, we will approve or decline the request
		//given the manager provides the correct value.
