...
public class Driver {
	...
	private static void getGBears() {
		...
		cr.add(Restrictions.isNotNull("bearType"));
		cr.add(Restrictions.like("name", "G%"));
		
		List<Animal> results = cr.list();
		
		System.out.println(results.toString());
		
	}
	
	private static void getWingspanBat(int low, int high) {
		Session session = HibernateUtil.getSession();
		Criteria cr = session.createCriteria(Bat.class);
		
		cr.add(Restrictions.isNotNull("wingspan"));
		cr.add(Restrictions.between("wingspan", low, high));
	...
	private static void getWingspanTotal() {
		Session session = HibernateUtil.getSession();
		Criteria cr = session.createCriteria(Bat.class);
		
		cr.add(Restrictions.isNotNull("wingspan"));
		cr.setProjection(Projections.sum("wingspan"));
		
		List results = cr.list();
		
		System.out.println("Wingspan total: " + results.toString());
		
	} 
	
	private static void getNumBears() {
		Session session = HibernateUtil.getSession();
		Criteria cr = session.createCriteria(Bear.class);
		...
		System.out.println("Num bears " + results.toString());
