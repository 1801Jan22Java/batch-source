...
BEGIN
    ...
    INSERT INTO TRANSACTION_HISTORY(ACCOUNT_ID, BALANCE_CHANGE, TRANSACTION_DATE, TRANSACTION_ID)
    VALUES (NEW.ACCOUNT_ID, NEW.BALANCE_CHANGE-OLD.BALANCE_CHANGE, TRANSACTIONDATE, TRANSACTION_ID_SEQUENCE.NEXTVAL);
END;
/
--STORED PROCEDURS

CREATE OR REPLACE PROCEDURE REGISTER_USER
(USERNAME IN VARCHAR2, UPASSWORD IN VARCHAR2, FIRST_NAME IN VARCHAR, lAST_NAME IN VARCHAR, USTATE IN VARCHAR, UZIP IN VARCHAR, BDAY IN DATE, UADDRESS IN VARCHAR2, USSN IN VARCHAR)
IS
    UID NUMBER;
BEGIN
    INSERT INTO USERS(USER_ID, SUPER_USER, USER_NAME, PASS_WORD) VALUES(42,0, USERNAME, UPASSWORD);
    SELECT USER_ID INTO UID FROM USERS WHERE USER_NAME = USERNAME;
