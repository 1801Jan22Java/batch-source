...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	public static final String filename = "C:\\Users\\Leonard\\GitRepos\\batch-source\\Project1_Reimbursment\\connection.properties";
								...
								// and copy/pasting the file to every folder in the project
	...
	public void addEmployee(Employee emp) {
		try {
			...
			if(rs.next()) {
				if(emp.isManager()) {
					emp = new Manager(rs.getInt("EmpID"), emp.getFirstName(), emp.getLastName(), 
							emp.getEmail(), emp.getPassword(), emp.getReportsTo(), 
							emp.getPhone(), emp.getJobTitle(), true, emp.getActive());	
				}
				else {
					emp = new Employee(rs.getInt("EmpID"), emp.getFirstName(), emp.getLastName(), 
							emp.getEmail(), emp.getPassword(), emp.getReportsTo(), 
							emp.getPhone(), emp.getJobTitle(), false, emp.getActive());
				}
			}
	...
	public void updateEmployee(Employee emp) {
		try {
			Connection con = ConnectionUtil.getConnectionFromFile(filename);
			String sql = "UPDATE EMPLOYEE SET EMAIL = ?, Pword = ?, IsManager = ?, FirstName = ?, LastName = ?, "
					+ "Phone = ? JobTitle = ? ReportsTo = ? Active = ?"
					+ "WHERE EmpID = ?";
			PreparedStatement ps = con.prepareStatement(sql);
			int manager = 0;
			if(emp.isManager())
			...
			String str = "SELECT * FROM EMPLOYEE";
				...
				while(rs.next()) {
					...
					if(isManager) {
						 ...
						 employees.add(new Manager(EmployeeID, FirstName, LastName, 
								email, password, reportsTo, phone, jobTitle, isManager, active));
					}
					else {
		...
		try {
			...
			String str = "SELECT * FROM EMPLOYEE WHERE EMPID = ?";
				...
				if(rs.next()) {
					...
					if(isManager) {
						con.close();
						return new Manager(EmployeeID, FirstName, LastName, 
								email, password, reportsTo, phone, jobTitle, isManager, active);
					}
					else {
						con.close();
